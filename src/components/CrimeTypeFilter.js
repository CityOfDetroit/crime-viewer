...
const CrimeTypeFilter = ({ crimeTypes, setCrimeTypes }) => {
  let byAreas = _.groupBy(
    _.uniqBy(Object.values(arrestCodes), (ac) => ac.area + ac.category),
    "area"
  );
                ...
                onChange={(e, d) => {
                  ...
                  Object.keys(byAreas).forEach((r) => {
                    if (r !== area) {
                      ...
                      changedCrimeTypes[area] = d.value.map(
                        (v) => v.split("_")[1]
                      );
                    }
                  });
